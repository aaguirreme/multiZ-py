...
Description:  This is an experimental module to define the base classes and
...
floattypes = (float, np.complex128)
complextypes = (complex, np.complex128)
scalartypes = inttypes + floattypes + complextypes
...
class mnumber:
    ...
    def __init__(self, coef_list):
        ...
        if type(coef_list) in scalartypes:
            ...
            self.coeffs = np.array([coef_list], np.complex128)
            ...
            coeffs = np.zeros(numcoeffs, np.complex128)
    ...
    def change_order(self, new_order):
        ...
        new_coeffs = np.zeros(new_n, np.complex128)  # temp. storage.
    ...
    def conjugate(self, imagDir):
        ...
        n = self.numcoeffs
        new_coeffs = np.zeros(n, np.complex128)      # Allocate space.
...
class marray:
    ...
    def __init__(self, coef_list):
        ...
        # Allocate space for coefficients.
        coeffs = np.zeros(shape + (numcoeffs,), np.complex128)
    ...
    def __setpos(self, index, value):
        ...
        # Case 2. Assign mcomplex/mdual to position.
            ...
            if (n1 > n2):                       # If value has lower order
                ...
                temp = np.zeros(shape + (n2,), np.complex128)
    ...
    def change_order(self, new_order):
        ...
        new_n = 1 << new_order
        new_coeffs = np.zeros(shape + (new_n,), np.complex128)
